...
summary(LiveAgent2$hour_created)
...
# convert to decimal time
login <- hour(LiveAgent2$AZ_time3) + minute(LiveAgent2$AZ_time3)/60 + second(LiveAgent2$AZ_time3)/3600
...
LiveAgent2_Idle <- ggplot(LiveAgent2, aes(x=TimeIdle/60, y=hour_created)) + 
  geom_point(aes(y = hour_created, colour = "hour_created")) +
  geom_point(aes(y = login, colour = "login")) +
  ylab("Hour") +
